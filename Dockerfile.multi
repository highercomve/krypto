...
RUN apk update; apk add git
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /go/bin/linux_amd64/krypto -v .

# build armv6 linux static
FROM src as linux_armv6

RUN apk update; apk add git
RUN CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=5 go build -o /go/bin/linux_armv6/krypto -v .

# build windows i386 static
FROM src as windows_386

RUN apk update; apk add git
RUN CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -o /go/bin/windows_386/krypto.exe -v .

# build windows amd64 static
FROM src as windows_amd64

RUN apk update; apk add git
RUN CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o /go/bin/windows_amd64/krypto.exe -v .

# build darwin amd64 static
FROM src as darwin_amd64

RUN apk update; apk add git
RUN CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o /go/bin/darwin_amd64/krypto -v .
